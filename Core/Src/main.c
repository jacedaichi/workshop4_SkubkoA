...
/* USER CODE BEGIN PV */
uint8_t rx_byte; //variable for saving byte
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart3, &rx_byte, 1); //enabling UART waiting for byte
  /* USER CODE END 2 */
	...
	if(huart->Instance == USART3)
	{
		switch(rx_byte)
		{
			...
			case '4': // Toggle Blue LED
				...
				break;
			case '+': // Toggle All LEDs
				HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15, GPIO_PIN_SET);
				HAL_UART_Transmit(&huart3, (uint8_t *)"All LEDs ON\r\n", 13, 100);
				break;
			case '-': // Disable All LEDs
				HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15, GPIO_PIN_RESET);
				HAL_UART_Transmit(&huart3, (uint8_t *)"All LEDs OFF\r\n", 14, 100);
				break;
			default: // If unkwn letter pressed
				HAL_UART_Transmit(&huart3, (uint8_t *)"Unknown letter\r\n", 17, 100);
				break;
		...
		HAL_UART_Receive_IT(&huart3, &rx_byte, 1); //enable reciving next byte after 1
	}
